...
BeginPackage["REAP`RGE2HDM`",{"REAP`RGESymbol`", "REAP`RGESolver`","REAP`RGEUtilities`", "REAP`RGETakagi`","REAP`RGEParameters`","REAP`RGEInitial`","MixingParameterTools`MPT3x3`",
...
RGERegisterModel["2HDM","REAP`RGE2HDM`",
	`Private`GetParameters,
        `Private`SolveModel,
        {RGEMd->`Private`GetMd,RGEMu->`Private`GetMu,RGEM\[Nu]->`Private`GetM\[Nu],RGEMe->`Private`GetMe,RGE\[Alpha]->`Private`Get\[Alpha],RGERaw->`Private`GetRawSolution,RGEYu->`Private`GetYu,RGEAll->`Private`GetSolution,RGEY\[Nu]->`Private`GetY\[Nu],RGEYe->`Private`GetYe,RGEYd->`Private`GetYd,RGETwistingParameters->`Private`GetTwistingParameters,RGERawM\[Nu]r->`Private`GetRawM\[Nu]r,RGERawY\[Nu]->`Private`GetRawY\[Nu],RGE\[Kappa]2->`Private`Get\[Kappa]2,RGECoupling->`Private`GetCoupling,RGEM\[Nu]r->`Private`GetM\[Nu]r,RGEM1Tilde->`Private`GetM1Tilde,RGEPoleMTop->`Private`GetPoleMTop,RGE\[Kappa]1->`Private`Get\[Kappa]1,RGEMixingParameters->`Private`GetMixingParameters,RGE\[Lambda]->`Private`Get\[Lambda]},
{{"MSSM",`Private`TransMSSM},{"2HDM0N",`Private`Trans2HDM0N},{"2HDM",`Private`Trans2HDM}},
        `Private`GetInitial,
...
Begin["`Private`"];
Map[Needs,{"REAP`RGESymbol`", "REAP`RGESolver`","REAP`RGEUtilities`","REAP`RGETakagi`", "REAP`RGEParameters`","REAP`RGEInitial`","MixingParameterTools`MPT3x3`","REAP`RGESM`","REAP`RGE2HDM0N`"}];
...
ParameterSymbols={RGEg1,RGEg2,RGEg3,RGEYu,RGEYd,RGEYe,RGEY\[Nu],RGE\[Kappa]1,RGE\[Kappa]2,RGEM\[Nu]r,RGE\[Lambda]1,RGE\[Lambda]2,RGE\[Lambda]3,RGE\[Lambda]4,RGE\[Lambda]5};
...
{"GUT",{
	...
	RGEM\[Nu]r->RGEGetM[RGE\[Theta]12,RGE\[Theta]13,RGE\[Theta]23,RGE\[Delta],RGE\[Delta]e,RGE\[Delta]\[Mu],RGE\[Delta]\[Tau],RGE\[CurlyPhi]1,RGE\[CurlyPhi]2,RGEMlightest,RGE\[CapitalDelta]m2atm,RGE\[CapitalDelta]m2sol,RGEMassHierarchy,RGEv\[Nu],RGEY\[Nu]],
	RGE\[Lambda]1->0.75,
	RGE\[Lambda]2->0.75,
	RGE\[Lambda]3->0.2,
	RGE\[Lambda]4->0.2,
...
(* calculate the new parameters *)
	...
	Return[{RGEg1->lg1,RGEg2->lg2,RGEg3->lg3,RGEYu->lYu,RGEYd->lYd,RGEYe->lYe,RGEY\[Nu]->lY\[Nu],RGE\[Kappa]1->l\[Kappa]1,RGE\[Kappa]2->l\[Kappa]2,RGEM\[Nu]r->lM\[Nu]r,RGE\[Lambda]1->l\[Lambda]1,RGE\[Lambda]2->l\[Lambda]2,RGE\[Lambda]3->l\[Lambda]3,RGE\[Lambda]4->l\[Lambda]4,RGE\[Lambda]5->l\[Lambda]5}];
];
