...
RGERegisterModel["SM","REAP`RGESM`",
	`Private`GetParameters,
        `Private`SolveModel,
        {RGEMe->`Private`GetMe,RGERawY\[Nu]->`Private`GetRawY\[Nu],RGE\[Lambda]->`Private`Get\[Lambda],RGEYu->`Private`GetYu,RGEAll->`Private`GetSolution,RGE\[Alpha]->`Private`Get\[Alpha],RGE\[Epsilon]1->`Private`Get\[Epsilon]1,RGE\[Kappa]->`Private`Get\[Kappa],RGE\[Epsilon]->`Private`Get\[Epsilon],RGECoupling->`Private`GetCoupling,RGEYd->`Private`GetYd,RGEM\[Nu]->`Private`GetM\[Nu],RGEMixingParameters->`Private`GetMixingParameters,RGERaw->`Private`GetRawSolution,RGERawM\[Nu]r->`Private`GetRawM\[Nu]r,RGEMd->`Private`GetMd,RGE\[Epsilon]Max->`Private`Get\[Epsilon]Max,RGEY\[Nu]->`Private`GetY\[Nu],RGEM1Tilde->`Private`GetM1Tilde,RGE\[Epsilon]1Max->`Private`Get\[Epsilon]1Max,RGEPoleMTop->`Private`GetPoleMTop,RGEMu->`Private`GetMu,RGEYe->`Private`GetYe,RGETwistingParameters->`Private`GetTwistingParameters,RGEM\[Nu]r->`Private`GetM\[Nu]r},
{{"MSSM",`Private`TransMSSM},{"SM",`Private`TransSM},{"SM0N",`Private`TransSM0N}},
        `Private`GetInitial,
